  ...
  try
    ...
    let rec collect acc =
      ...
      | Sqlite3.Rc.ROW ->
          ...
          let get_opt_float i = match row i with Sqlite3.Data.FLOAT f -> Some f | Sqlite3.Data.NULL -> None | _ -> None in
          let consumable : Consumable.consumable = {
            ...
            Consumable.purchase_price = get_opt_float 7;
            Consumable.created_at = Id.of_int64 (Int64.of_int (get_int 8));
            Consumable.updated_at = Id.of_int64 (Int64.of_int (get_int 9));
          } in
...
let get_by_id consumable_id =
  let conn = Database.open_db () in
  try
    let stmt = Sqlite3.prepare conn "SELECT * FROM consumables WHERE id = ? LIMIT 1" in
...
let update_quantity consumable_id delta transaction_type notes =
  let conn = Database.open_db () in
  try
    let stmt = Sqlite3.prepare conn {|
    ...
    Sqlite3.bind stmt 3 (Sqlite3.Data.INT consumable_id) |> ignore;
    ignore (Sqlite3.step stmt);
    ignore (Sqlite3.finalize stmt);

    let transaction_stmt = Sqlite3.prepare conn {|
      INSERT INTO consumable_transactions (id, consumable_id, transaction_type, quantity, date, notes, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    |} in
    let transaction_id = Id.generate () in
    let now = Timestamp.now () in
    Sqlite3.bind transaction_stmt 1 (Sqlite3.Data.INT transaction_id) |> ignore;
    Sqlite3.bind transaction_stmt 2 (Sqlite3.Data.INT consumable_id) |> ignore;
    Sqlite3.bind transaction_stmt 3 (Sqlite3.Data.TEXT transaction_type) |> ignore;
    Sqlite3.bind transaction_stmt 4 (Sqlite3.Data.INT (Int64.of_int delta)) |> ignore;
    Sqlite3.bind transaction_stmt 5 (Sqlite3.Data.INT now) |> ignore;
    Sqlite3.bind transaction_stmt 6 (Sqlite3.Data.TEXT (Option.value notes ~default:"")) |> ignore;
    Sqlite3.bind transaction_stmt 7 (Sqlite3.Data.INT now) |> ignore;
    ignore (Sqlite3.step transaction_stmt);
    ignore (Sqlite3.finalize transaction_stmt);
...
let delete consumable_id =
  let conn = Database.open_db () in
  try
    let stmt = Sqlite3.prepare conn "DELETE FROM consumable_transactions WHERE consumable_id = ?" in
    Sqlite3.bind stmt 1 (Sqlite3.Data.INT consumable_id) |> ignore;
    ignore (Sqlite3.step stmt);
    ignore (Sqlite3.finalize stmt);
...
let get_history consumable_id =
  let conn = Database.open_db () in
  try
    let stmt = Sqlite3.prepare conn "SELECT * FROM consumable_transactions WHERE consumable_id = ? ORDER BY date DESC" in
    Sqlite3.bind stmt 1 (Sqlite3.Data.INT consumable_id) |> ignore;
    let rec collect acc =
      match Sqlite3.step stmt with
      | Sqlite3.Rc.DONE -> acc
      | Sqlite3.Rc.ROW ->
          ...
          let get_string i = match row i with Sqlite3.Data.TEXT s -> s | _ -> "" in
          let get_opt_string i = match row i with Sqlite3.Data.TEXT s -> Some s | Sqlite3.Data.NULL -> None | _ -> None in
          let transaction : Consumable.consumable_transaction = {
            Consumable.id = Id.of_int64 (Int64.of_int (get_int 0));
            Consumable.consumable_id = Id.of_int64 (Int64.of_int (get_int 1));
            Consumable.transaction_type = get_string 2;
            Consumable.quantity = get_int 3;
            Consumable.date = Id.of_int64 (Int64.of_int (get_int 4));
            Consumable.notes = get_opt_string 5;
            Consumable.created_at = Id.of_int64 (Int64.of_int (get_int 6));
          } in
          collect (transaction :: acc)
      | _ -> raise (Failure "Unexpected sqlite result")
    in
    let result = collect [] in
    ignore (Sqlite3.finalize stmt);
